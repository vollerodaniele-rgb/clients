/* Idea box moderation (shared)
   ------------------------------------------------------------
   One copy for every box. Which box comes from the folder in the
   URL. Uses the same key as the rest of the dashboard.
   ------------------------------------------------------------ */
function currentBox() {
...
async function fetchIdeas(state) {
  const url = `https://api.github.com/repos/${OWNER}/${REPO}/issues` +
    `?labels=${encodeURIComponent(LABEL + "," + BOX_LABEL)}&state=${state}&sort=created&direction=desc&per_page=100`;
