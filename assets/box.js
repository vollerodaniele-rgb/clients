/* Idea box (shared)
   ------------------------------------------------------------
   One copy for every box. Which box comes from the folder in the
   URL, its wording from boxes/<slug>.json, and its ideas are
   issues labelled idea and box:<slug> so boxes never mix.
   ------------------------------------------------------------ */
function currentBox() {
...
/* Every word on the page comes from the data file, so one template
   serves every box. */
async function loadWording() {
  try {
    const res = await fetch(`../../boxes/${BOX}.json`, { cache: "no-store" });
    if (!res.ok) throw new Error(String(res.status));
    const d = await res.json();

    if (d.kicker) $("kicker").textContent = d.kicker;
    if (d.title) $("box-title").textContent = d.title;
    if (d.intro) $("intro").textContent = d.intro;
    if (d.placeholder) $("idea-text").placeholder = d.placeholder;
    if (d.footer) $("foot").textContent = d.footer;
    if (d.wall) $("wall-title").textContent = d.wall;

    document.title = (d.kicker ? d.kicker + " | " : "") + (d.title || "Idea Box");
  } catch (err) {
    console.error("box wording failed:", err);
  }
}
...
async function loadIdeas() {
  ...
  try {
    ...
    const res = await fetch(`${CONFIG.submitUrl}/ideas?site=${CONFIG.site}&client=${encodeURIComponent(BOX)}`, {
      cache: "no-store"
    });
...
function setupForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const payload = { site: CONFIG.site, client: BOX, idea, name, website: $("idea-website").value };
